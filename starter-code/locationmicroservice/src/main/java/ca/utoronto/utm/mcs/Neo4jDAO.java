...
public class Neo4jDAO {
    ...
    public Result getDriver(String uid) {
        String query = "MATCH (n: user {uid: '%s', is_driver: true}) RETURN n.longitude, n.latitude, n.street";
        query = String.format(query, uid);
        return this.session.run(query);
    }
    ...
    public Result getUserInRadius(Double latitude, Double longitude, int radius) {
        String query = "MATCH (n: user {is_driver: true}) WHERE ((n.latitude-%f)^2 + (n.longitude-%f)^2) <= %d^2 RETURN n.uid, n.longitude, n.latitude, n.street";
    ...
    public Result getRoadId(Long id) {
        String query = "MATCH (n: road) where ID(n) = %d RETURN n.name, n.has_traffic";
        query = String.format(query, id);
        return this.session.run(query);
    }

    public Result getNavigationRoute(String driverRoadName, String passengerRoadName) {
        String query = "MATCH (source: road {name: '%s'}), (target: road {name: '%s'})\n" +
                ...
                "RETURN totalCost AS tc, nodeIds AS nodeIds, costs AS costs";
        query = String.format(query, driverRoadName, passengerRoadName);
        return this.session.run(query);
    }
